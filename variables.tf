...
variable "ami_id" {
  description = "The ID of the AMI to run in this cluster. Should be an AMI that has consul installed at /usr/local/bin/consul."
  type        = string
  default = ""
}

variable "instance_type" {
  description = "The type of EC2 Instances to run for each node in the cluster (e.g. t2.micro). This setting overrides the instance_size variable which uses the recommended instance types."
  type        = string
  default     = ""
}

variable "instance_size" {
  description = "Defaults to 'extra_small', which is m5.large. Other valid options are small (m5.xlarge), large (m5.2xlarge) and extra_large (m5.4xlarge). This value is ignored if instance_type is specified."
  type        = string
  default     = "extra_small"
}
...
variable "cluster-tag-key" { default = "" }
variable "cluster-tag-value" { default = "" }
...
variable "enable-gossip-encryption" { default = "" }
variable "gossip-encryption-key" { default = "" }
variable "enable-rpc-encryption" { default = "" }
